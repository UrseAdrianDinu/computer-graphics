...
void Lab3::Update(float deltaTimeSeconds)
{
    ...
    modelMatrix = glm::mat3(1);
    modelMatrix *= transform2D::Translate(permx, permy);
    modelMatrix *= transform2D::Translate(cx, cy);
    modelMatrix *= transform2D::Rotate(rotp);
    modelMatrix *= transform2D::Translate(-cx, -cy);
    RenderMesh2D(meshes["square4"], shaders["VertexColor"], modelMatrix);

    modelMatrix *= transform2D::Translate(100, 0);
    modelMatrix *= transform2D::Scale(0.3f, 1.0f);
...
void Lab3::OnInputUpdate(float deltaTime, int mods)
{
    if (window->KeyHold(GLFW_KEY_W)) {
        permx += deltaTime * 100.0f * cos(rotp);
        permy += deltaTime * 100.0f * sin(rotp);
    }
