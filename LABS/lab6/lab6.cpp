...
Mesh* Lab6::CreateMesh(const char *name, const std::vector<VertexFormat> &vertices, const std::vector<unsigned int> &indices)
{
    ...
    // Daca inversez color pipe cu normal pipe in aceasta functie, culorile sunt schimbate doar pentru cubul
    // ce este creat folosind VertexNormal
    // Daca fac acesta schimbare in shader, atunci celelalte 2 cuburi sunt modificate
    // (sunt create cu labShader)
