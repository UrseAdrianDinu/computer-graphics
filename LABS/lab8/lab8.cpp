...
void Lab8::Init()
{
    ...
    // Light & material properties
    {
        ...
        Ke = glm::vec3(0.4, 0.5, 0.9);
        Ka = glm::vec3(0.9, 0.3, 0.4);
        Kd = glm::vec3(1, 0, 0.0);
        Ks = glm::vec3(0.9, 0.4, 0.5);
    }
}
...
void Lab8::Update(float deltaTimeSeconds)
{
    ...
    // Render ground
    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(0.5,0.5,1));
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    int material_kd = glGetUniformLocation(shader->program, "Kd");
    glUniform3f(material_kd, Kd.r,Kd.g,Kd.b);

    int material_ks = glGetUniformLocation(shader->program, "Ks");
    glUniform3f(material_ks, Ks.r, Ks.g, Ks.b);

    int material_ka = glGetUniformLocation(shader->program, "Ka");
    glUniform3f(material_ka, Ka.r, Ka.g, Ka.b);

    int material_ke = glGetUniformLocation(shader->program, "Ke");
    glUniform3f(material_ke, Ke.r, Ke.g, Ke.b);
